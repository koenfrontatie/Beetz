...
internal sealed class CustomAssetImporter : AssetPostprocessor
{
    ...
    private void OnPreprocessAudio()
    {
        ...
        sampleSettings.compressionFormat = AudioCompressionFormat.PCM; //alternatives: .AAC, .ADPCM, .GDADPCM, .HEVAG, .MP3, .PCM, .VAG, .XMA
        sampleSettings.quality = 1; //ranging from 0 (0%) to 1 (100%), currently set to 1%, wich is the smallest value that can be set in the inspector | Probably only useful when the compression format is set to Vorbis
