...
public class Reverb : AudioEffect
{
    ...
    private readonly AllPassFilter[] _allpassFiltersL;
    private readonly AllPassFilter[] _allpassFiltersR;
    private readonly float _gain;
    private readonly float _wet1;
    private readonly float _wet2;
    private readonly float _dry;

    int[] combDelayLengthsL = new int[] { 1116, 1188, 1277, 1356 };
    int[] combDelayLengthsR = new int[] { 1116 + 23, 1188 + 19, 1277 + 17, 1356 + 13 };
    ...
    public Reverb(float gain, float wet1, float wet2, float dry)
    {
        ...
        _combFiltersL = new CombFeedbackFilter[combDelayLengthsL.Length];
        _combFiltersR = new CombFeedbackFilter[combDelayLengthsR.Length];
        for (int i = 0; i < combDelayLengthsL.Length; i++)
        {
            _combFiltersL[i] = new CombFeedbackFilter(combDelayLengthsL[i], 0.84f, 0.84f);
            _combFiltersR[i] = new CombFeedbackFilter(combDelayLengthsR[i], 0.84f, 0.84f);
        }

        // Initialize all-pass filters
        _allpassFiltersL = new AllPassFilter[allpassDelayLengths.Length];
        _allpassFiltersR = new AllPassFilter[allpassDelayLengths.Length];
        for (int i = 0; i < allpassDelayLengths.Length; i++)
        {
            _allpassFiltersL[i] = new AllPassFilter(allpassDelayLengths[i]);
            _allpassFiltersR[i] = new AllPassFilter(allpassDelayLengths[i]);
        }
    }

    public override DiscreteSignal ApplyTo(DiscreteSignal signal, FilteringMethod method = FilteringMethod.Auto)
    {
        var outputL = new float[signal.Length];
        var outputR = new float[signal.Length];

        for (int n = 0; n < signal.Length; n++)
        {
            float outL = 0, outR = 0;
            float input = signal[n] * _gain;
            ...
            {
                outL = _allpassFiltersL[i].Process(outL);
                outR = _allpassFiltersR[i].Process(outR);
            }

            // Calculate output REPLACING anything already there
            outputL[n] = outL * _wet1 + signal[n] * _dry;
            outputR[n] = outR * _wet2 + signal[n] * _dry;
        }

        // Merge left and right channels for stereo output
        var output = new float[signal.Length];
        for (int n = 0; n < signal.Length; n++)
        {
            output[n] = (outputL[n] + outputR[n]) / 2.0f;
        }
    ...
    public override void Reset()
    {
        ...
        foreach (var combFilter in _combFiltersL)
        {
            combFilter.Reset();
        }

        foreach (var combFilter in _combFiltersR)
        {
            combFilter.Reset();
        }

        // Reset state of all-pass filters
        foreach (var allpassFilter in _allpassFiltersL)
        {
            allpassFilter.Reset();
        }

        foreach (var allpassFilter in _allpassFiltersR)
        {
            allpassFilter.Reset();
        }
    }
...
public class AllPassFilter : FirFilter
{
    public AllPassFilter(int delay) : base(DesignFilter.FirWinFdAp(1, delay))
    {
    }

    public override float Process(float input)
    {
        return ApplyTo(new DiscreteSignal(44100, new[] { input })).Samples[0];
    }
}
