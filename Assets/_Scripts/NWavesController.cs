...
public class NWavesController : MonoBehaviour
{
    ...
    //    var reverb = new Reverb(1f, .9f, .9f, .5f);


    //    var outputSignal = reverb.ApplyTo(waveFile.Signals[0]);
    ...
    public async Task StartNwavesFree()
    {
        WaveFile waveFile;
        ...
        // Adjust the input signal length to be a multiple of 4 (if needed)
        int numSamples = waveFile[Channels.Left].Length;
        if (waveFile.WaveFmt.ChannelCount == 1)
        {
            var paddedSamples = new float[numSamples * 4 - numSamples];
        ...
        else
        {
            ...
            var padding = new DiscreteSignal(waveFile.Signals[0].SamplingRate, paddedSamples, true);
            waveFile.Signals[0] = waveFile.Signals[0].Concatenate(padding);
            waveFile.Signals[1] = waveFile.Signals[1].Concatenate(padding);
        }

        // Apply the reverb effect
        var reverb = new Reverb(1f, 0.9f, 0.9f, 0.5f);
        var outputSignalLeft = reverb.ApplyTo(waveFile.Signals[0]);
        var outputSignalRight = reverb.ApplyTo(waveFile.Signals[1]);

        // Replace the original signals with the reverberated signals
        waveFile.Signals[0] = outputSignalLeft;
        waveFile.Signals[1] = outputSignalRight;
